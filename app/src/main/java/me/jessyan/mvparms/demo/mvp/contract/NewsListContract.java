...
public interface NewsListContract {
  ...
  interface View extends IView {
    void loadListData(List<NewsListEntity.TngouBean> mList,boolean isSuccess);
  }

  //Model层定义接口,外部只需关心model返回的数据,无需关心内部细节,及是否使用缓存
  interface Model extends IModel {
    Observable<NewsListEntity> getListData(String url,String cacheName ,int page, int id,boolean update);
  }
}