...
@ActivityScope
public class ImageListPresenter extends
    BasePresenter<ImageListContract.Model, ImageListContract.View> {
  ...
  public void requestList(String cacheName , int id,boolean update){
    Timber.e("requestList*****************************",cacheName);
    if (update) page = 1;
    mModel.getListData("/tnfs/api/list",cacheName,page,id,update)
        ...
        .subscribe(new SuccessSubscriber<ImageEntity>(mErrorHandler) {
          @Override
          public void onSuccess(ImageEntity entity) {
            Timber.e("requestList*********",entity.getTngou().size());
            page++;
